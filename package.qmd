...
2. Only include folder 4 with restricted data if an exemption to the  {{< var ej.Policy >}} has been granted to you and you are providing the replication team with temporary access to the data; all codes and unrestricted data should go to folder 3.
:::
...
As indicated in the journal's [ {{< var ej.Policy >}}](https://res.org.uk/replication-policy/) all packages should include the following information: 
...
<!--
You will be *invited* to submit your package via an upload link by the Data Editor. This invitation will be generated *after* you have submitted the following items via [The {{< var ej.short >}} Editorial Express](https://editorialexpress.com/ectj) as a resubmission of your paper:
-->
...
2. `2-onlineappendix` (see @sec-prelims)
3. `3-replication-package.zip` (see @sec-prelims)
3. Signed and dated [checklist.pdf](checklist.pdf)

Notice that you will submit `1-paper` and `2-onlineappendix` twice, once on Editorial Express to trigger the invitation link, and once as part of your replication package (see @sec-prelims).
...
Once the reproducibility checks are concluded, and upon invitation of the Data Editor, you will be requested to perform a final step:  publish your replication package at the {{< var ej.long >}}'s [community of Zenodo]({{< var ej.zenodo >}}). To do so, you need to do the following: 

1. Sign up [here](https://zenodo.org/signup/) for a free zenodo account, if you don't have one already.
2. Go to the {{< var ej.long >}}'s community of Zenodo at [{{< var ej.zenodo >}}]({{< var ej.zenodo >}}).
